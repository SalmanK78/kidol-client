...
const Register = () => {
  ...
  const handleRegister = event => {
      ...
      if(password !== '') {
          if (password !== confirm) {
              setError('Password does not mach')
              return;
          } else if (password.length < 6) {
              setError('Password length must be 6 charecter');
              return;
          }
          form.reset()
          navigate('/')
  ...
  return (
    <div>
      <Header login={true}>Register</Header>
    <div className="max-w-md mx-auto py-5 px-5 md:px-20">
          <form onSubmit={handleRegister} className="bg-white">
            ...
            <span className="text-sm ml-2 cursor-pointer">
              <Link to="/login">
                <span>Already have an acount </span><button className="btn-link  hover:text-blue-500"> Login</button>
              </Link>
            </span>
          </form>
    </div>
